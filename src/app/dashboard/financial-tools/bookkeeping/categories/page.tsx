    ...
    <div className="flex-1 p-6">
      ...
      <div className="mb-8 p-6 bg-white shadow-card rounded-card">
        ...
        <form action={editingCategory ? updateAction : createAction} className="space-y-4">
          ...
          <div>
            ...
            <input type="text" id="name" name="name" defaultValue={editingCategory?.name || ''} required className="mt-1 block w-full rounded-control border-clay-200 shadow-sm" />
          </div>
          <div>
            <label htmlFor="type" className="block text-sm font-medium text-clay-700">Type</label>
            <select id="type" name="type" defaultValue={editingCategory?.type || 'expense'} required className="mt-1 block w-full rounded-control border-clay-200 shadow-sm">
          ...
          <div className="flex items-center">
            ...
            {editingCategory && (
              <button type="button" onClick={() => setEditingCategory(null)} className="ml-4 inline-flex justify-center py-2 px-4 border border-clay-200 shadow-sm text-sm font-medium rounded-control text-clay-700 bg-white hover:bg-clay-50">
                Cancel
              </button>
            )}
          </div>
      ...
      {loading ? (
        ...
        <div className="bg-white p-6 rounded-card shadow-card">
          <h2 className="font-display text-xl font-semibold text-clay-800 mb-4">Categories</h2>
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-clay-200">
              <thead className="bg-clay-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-clay-500 uppercase">Name</th>
