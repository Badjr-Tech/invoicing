    ...
    <div className="flex-1 p-6">
      ...
      <div className="mb-8 p-6 bg-white shadow-card rounded-card">
        ...
        <form action={editingTransaction ? updateAction : createAction} className="space-y-4">
          ...
          <div>
            ...
            <input type="text" id="description" name="description" defaultValue={editingTransaction?.description || ''} required className="mt-1 block w-full rounded-control border-clay-200 shadow-sm" />
          </div>
          <div>
            <label htmlFor="amount" className="block text-sm font-medium text-clay-700">Amount</label>
            <input type="number" id="amount" name="amount" defaultValue={editingTransaction?.amount || ''} required className="mt-1 block w-full rounded-control border-clay-200 shadow-sm" />
          </div>
          <div>
            <label htmlFor="type" className="block text-sm font-medium text-clay-700">Type</label>
            <select id="type" name="type" defaultValue={editingTransaction?.type || 'expense'} required className="mt-1 block w-full rounded-control border-clay-200 shadow-sm">
              <option value="income">Income</option>
              <option value="expense">Expense</option>
            </select>
          </div>
          <div>
            <label htmlFor="categoryId" className="block text-sm font-medium text-clay-700">Category</label>
            <select id="categoryId" name="categoryId" defaultValue={editingTransaction?.categoryId || ''} className="mt-1 block w-full rounded-control border-clay-200 shadow-sm">
          ...
          <div>
            <label htmlFor="frequency" className="block text-sm font-medium text-clay-700">Frequency</label>
            <select id="frequency" name="frequency" defaultValue={editingTransaction?.frequency || 'monthly'} required className="mt-1 block w-full rounded-control border-clay-200 shadow-sm">
          ...
          <div>
            ...
            <input type="date" id="startDate" name="startDate" defaultValue={editingTransaction?.startDate ? new Date(editingTransaction.startDate).toISOString().split('T')[0] : ''} required className="mt-1 block w-full rounded-control border-clay-200 shadow-sm" />
          </div>
          <div>
            <label htmlFor="endDate" className="block text-sm font-medium text-clay-700">End Date (optional)</label>
          ...
          <div className="flex items-center">
            ...
            {editingTransaction && (
              <button type="button" onClick={() => setEditingTransaction(null)} className="ml-4 inline-flex justify-center py-2 px-4 border border-clay-200 shadow-sm text-sm font-medium rounded-control text-clay-700 bg-white hover:bg-clay-50">
                Cancel
              </button>
            )}
          </div>
      ...
      {loading ? (
        ...
        <div className="bg-white p-6 rounded-card shadow-card">
          <h2 className="font-display text-xl font-semibold text-clay-800 mb-4">Recurring Transactions</h2>
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-clay-200">
              <thead className="bg-clay-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-clay-500 uppercase">Description</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-clay-500 uppercase">Amount</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-clay-500 uppercase">Type</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-clay-500 uppercase">Frequency</th>
              ...
              <tbody className="bg-white divide-y divide-clay-200">
                {recurringTransactions.map(transaction => (
                  <tr key={transaction.id}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-clay-900">{transaction.description}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-clay-900">{transaction.amount}</td>
