...
export default function InvoicesPageClient({
  ...
  return (
    <div className="w-full p-6 bg-white rounded-card shadow-card">
      <h1 className="font-display text-3xl font-semibold text-clay-800 mb-6">Your Invoices</h1>
      ...
      <div className="mb-4 flex items-center">
        <input
          ...
          className="mr-2"
        />
        <label htmlFor="showArchived" className="text-sm font-medium text-clay-700">
          Show Archived Invoices
        </label>
      </div>

      {invoices.length === 0 ? (
        <p className="text-clay-600">No {showArchived ? 'archived' : 'active'} invoices found.</p>
      ) : (
          ...
          {invoices.map((invoice) => {
            ...
            return (
              <li key={invoice.id} className="p-4 bg-clay-50 rounded-card shadow">
                ...
                <p className="text-sm text-clay-600">Due Date: {invoice.dueDate ? new Date(invoice.dueDate).toLocaleDateString() : 'N/A'}</p>
