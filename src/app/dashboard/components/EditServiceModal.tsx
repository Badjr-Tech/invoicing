...
export default function EditServiceModal({ service, onClose, onSubmissionSuccess, businesses }: { service: Service, onClose: () => void, onSubmissionSuccess?: () => void, businesses: { id: number; businessName: string }[] }) {
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center">
      <div className="relative mx-auto p-5 border w-full max-w-md shadow-lg rounded-control bg-white">
        <div className="mt-3 text-center">
          <h3 className="text-lg leading-6 font-medium text-clay-900">Edit Service</h3>
          <div className="mt-2 px-7 py-3">
            <form action={formAction} className="space-y-6">
              ...
              <div className="items-center gap-2 mt-3 sm:flex">
                <button
                  type="submit"
                  className="w-full inline-flex justify-center rounded-control border border-transparent shadow-sm px-4 py-2 bg-ember-600 text-base font-medium text-white hover:bg-ember-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sage-300 sm:w-auto sm:text-sm"
                >
                  Save Changes
                </button>
                <button
