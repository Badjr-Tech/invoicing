...
export default function ContractorsClientPage() {
  ...
  return (
    <div className="p-6">
      <h1 className="font-display text-2xl font-semibold mb-6">Manage Contractors</h1>

      {/* Add/Edit Contractor Form */}
      <div className="bg-white shadow-card rounded-card p-6 mb-8">
        <h2 className="text-2xl font-semibold mb-4">{editingContractor ? 'Edit Contractor' : 'Add New Contractor'}</h2>
        {error && <div className="text-red-500 mb-4">{error}</div>}
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="name" className="block text-sm font-medium text-clay-700">Name</label>
            <input
              type="text"
              id="name"
              value={name}
          ...
          <div>
            <label htmlFor="email" className="block text-sm font-medium text-clay-700">Email</label>
            <input
              type="email"
              id="email"
              value={email}
          ...
          <div>
            <label htmlFor="role" className="block text-sm font-medium text-clay-700">Role (Optional)</label>
            <input
              type="text"
              id="role"
              value={role}
          ...
          <div>
            <label htmlFor="monthlyPayment" className="block text-sm font-medium text-clay-700">Monthly Payment ($)</label>
            <input
              type="number"
              id="monthlyPayment"
          ...
          <div>
            <label htmlFor="business" className="block text-sm font-medium text-clay-700">Assign to Business</label>
            <select
              id="business"
          ...
          <ul className="divide-y divide-clay-200">
            {contractors.map((contractor) => (
              <li key={contractor.id} className="py-4 flex justify-between items-center">
                <div>
                  <p className="text-lg font-medium text-clay-900">{contractor.name} ({contractor.role || 'N/A'})</p>
                  ...
                  {contractor.contractorTaxId && (
                    <p className="text-xs text-clay-500">Tax ID: {contractor.contractorTaxId}</p>
                  )}
                  {contractor.contractorAddress && (
                    <p className="text-xs text-clay-500">Address: {contractor.contractorAddress}, {contractor.contractorCity}, {contractor.contractorState} {contractor.contractorZipCode}</p>
                  )}
                  {contractor.onboardedAt ? (
                    <p className="text-xs text-sage-700">Onboarded: {new Date(contractor.onboardedAt).toLocaleDateString()}</p>
                  ) : (
                    <p className="text-xs text-yellow-600">Onboarding Pending</p>
                  )}
                  {contractor.w9Url && (
                    <p className="text-xs text-sage-700">W-9: <a href={contractor.w9Url} target="_blank" rel="noopener noreferrer" className="underline">View</a></p>
                  )}
                </div>
                <div className="flex space-x-2">
                  <button
                    onClick={() => handleEdit(contractor)}
                    className="inline-flex items-center px-3 py-1.5 border border-clay-200 shadow-sm text-sm font-medium rounded-control text-clay-700 bg-white hover:bg-clay-50"
                  >
                    Edit
                  </button>
                  <button
                    onClick={() => handleDelete(contractor.id)}
                    className="inline-flex items-center px-3 py-1.5 border border-transparent shadow-sm text-sm font-medium rounded-control text-white bg-red-600 hover:bg-red-700"
                  >
                    Delete
                  </button>
                  {/* Future Payment Button */}
                  <button
                    className="inline-flex items-center px-3 py-1.5 border border-transparent shadow-sm text-sm font-medium rounded-control text-white bg-sage-600 hover:bg-sage-700"
                  ...
                  {!contractor.onboardedAt && (
                    <button
                      ...
                      className="inline-flex items-center px-3 py-1.5 border border-transparent shadow-sm text-sm font-medium rounded-control text-white bg-ember-600 hover:bg-ember-500"
                    >
                      Invite to Onboard
                    </button>
                  )}
