...
import EditServiceModal from "@/app/dashboard/components/EditServiceModal";
...
export default function CategoryServicesList({ category, services, businesses }: { category: ServiceCategory, services: Service[], businesses: { id: number; businessName: string }[] }) {
  ...
  return (
    <div>
      <h2 className="font-display text-xl font-semibold text-clay-800 mb-4">Services</h2>
      <ul className="space-y-4">
        {services.length === 0 ? (
          <p>No services found in this category. Add one using the form!</p>
        ) : (
          services.map((service) => (
            <li key={service.id} className="p-4 bg-white rounded-card shadow flex justify-between items-center">
              <div>
                <p className="font-semibold">{service.name}</p>
                {service.description && <p className="text-sm text-clay-600">{service.description}</p>}
              ...
              <div className="flex items-center space-x-2">
                ...
                <button
                  ...
                  disabled={isPending}
                  className="px-3 py-1 bg-red-500 text-white rounded-control hover:bg-red-600 disabled:bg-gray-400"
                >
