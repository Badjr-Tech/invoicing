    ...
    <div className="flex-1 p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <select
          ...
          className="p-2 border border-clay-200 rounded-control"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div className="p-6 bg-sage-100 rounded-card shadow-card">
          <h2 className="text-xl font-bold text-sage-800">Total Income</h2>
          <p className="font-display text-2xl font-semibold text-sage-700">${income.toFixed(2)}</p>
        </div>
        <div className="p-6 bg-red-100 rounded-card shadow-card">
          <h2 className="text-xl font-bold text-red-800">Total Expenses</h2>
          <p className="font-display text-2xl font-semibold text-red-600">${expenses.toFixed(2)}</p>
        </div>
        <div className="p-6 bg-blue-100 rounded-card shadow-card">
          <h2 className="text-xl font-bold text-blue-800">Profit</h2>
          <p className="font-display text-2xl font-semibold text-sage-700">${profit.toFixed(2)}</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="p-6 bg-white rounded-card shadow-card">
          <h2 className="font-display text-xl font-semibold text-clay-800 mb-4">Add Income</h2>
        ...
        <div className="p-6 bg-white rounded-card shadow-card">
          <h2 className="font-display text-xl font-semibold text-clay-800 mb-4">Add Expense</h2>
          <form action={addTransactionFormAction} className="space-y-4">
            <input type="hidden" name="businessId" value={selectedBusinessId || ''} />
      ...
      <div className="mt-6 bg-white p-6 rounded-card shadow-card">
        <h2 className="font-display text-xl font-semibold text-clay-800 mb-4">Transactions</h2>
          ...
          <table className="min-w-full divide-y divide-clay-200">
            <thead className="bg-clay-50">
              <tr>
                ...
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-clay-500 uppercase tracking-wider">Description</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-clay-500 uppercase tracking-wider">Notes</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-clay-500 uppercase tracking-wider">Type</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-clay-500 uppercase tracking-wider">Amount</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-clay-200">
              {transactions.map(transaction => (
                <tr key={transaction.id}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-clay-500">{new Date(transaction.date).toLocaleDateString()}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-clay-900">{transaction.description}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-clay-900">{transaction.notes}</td>
                  <td className={`px-6 py-4 whitespace-nowrap text-sm ${transaction.type === 'income' ? 'text-sage-700' : 'text-red-600'}`}>{transaction.type}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-clay-900">${parseFloat(transaction.amount).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
