    ...
    <div className="flex-1 p-6">
      ...
      {courses.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {courses.map((course) => (
            <div key={course.id} className="bg-white p-6 rounded-card shadow-card">
              <h2 className="text-xl font-bold text-clay-900 mb-2">{course.title}</h2>
              <p className="text-clay-700 mb-4">{course.description || "No description provided."}</p>
              <p className="text-sm text-clay-500 mb-4">Status: <span className={`font-semibold ${course.status === 'published' ? 'text-sage-700' : 'text-yellow-600'}`}>{course.status}</span></p>
